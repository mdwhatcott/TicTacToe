...
(describe "Unbeatable AI"
  ...
  (it "suggests the only remaining (non-winning) move"
    (should= 8 (suggest X (vector->grid [O X X
                                         X O X
                                         X O _]))))
  ...
  (it "suggests the winning move from many options"
    (should= 6 (suggest X (vector->grid [X O O
                                         X _ _
                                         _ _ _]))))

  (it "suggests a winning move to prevent subsequent loss"
    (should= 6 (suggest O (vector->grid [X X _
                                         _ X _
                                         _ O O]))))

  (it "suggests a blocking move to prevent subsequent loss"
    (should= 6 (suggest O (vector->grid [_ _ X
                                         _ X _
                                         _ _ O])))

    (should= 6 (suggest O (vector->grid [_ O X
                                         _ X _
                                         _ _ _])))

    (should= 5 (suggest O (vector->grid [O _ _
                                         X X _
                                         _ O X]))))

  (it "suggests the best move available, such as a fork"
    (should= 4 (suggest X (vector->grid [X X O
                                         O _ _
                                         _ _ _]))))
